...
module.exports=async ctx=>{
  ...
  if(clear){
    const r=await ctx.broker('core_tasks','query',{
      text:`INSERT INTO tasks_tenant_openai_setting(tenant_id,api_key_ciphertext,api_key_iv,api_key_tag,model,updated_by_email)
  ...
  if(apiKey){
    ...
    const r=await ctx.broker('core_tasks','query',{
      text:`INSERT INTO tasks_tenant_openai_setting(
  ...
  const r=await ctx.broker('core_tasks','query',{
    text:`INSERT INTO tasks_tenant_openai_setting(tenant_id,model,updated_by_email)
          VALUES($1,$2,$3)
          ON CONFLICT(tenant_id) DO UPDATE
