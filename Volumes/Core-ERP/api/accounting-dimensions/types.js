  ...
  const r=await ctx.broker('core_erp','query',{
    text:`SELECT t.*,
            COALESCE((SELECT array_agg(tm.module_id ORDER BY m.sort_order,m.module_name)
              FROM erp_accounting_dimension_type_module tm JOIN erp_module m ON m.module_id=tm.module_id
              WHERE tm.accounting_dimension_type_id=t.accounting_dimension_type_id),'{}'::uuid[]) module_ids
          FROM erp_accounting_dimension_type t
