...
module.exports=async ctx=>{
  ...
  const id=nullable(ctx.body.accounting_dimension_type_id);
  ...
  const text=id
    ? `UPDATE erp_accounting_dimension_type
       SET type_code=$3,type_name=$4,schema_json=$5::jsonb,ui_schema_json=$6::jsonb,is_active=$7,updated_at=now()
       WHERE tenant_id=$1 AND organisation_id=$2 AND accounting_dimension_type_id=$8
       RETURNING *`
    : `INSERT INTO erp_accounting_dimension_type(tenant_id,organisation_id,type_code,type_name,schema_json,ui_schema_json,is_seeded,is_active)
       VALUES($1,$2,$3,$4,$5::jsonb,$6::jsonb,false,$7) RETURNING *`;
  const r=await ctx.broker('core_erp','query',{text,values:id?[...values,id]:values});
  if(!r.rowCount)return ctx.send(404,{error:'Accounting dimension type not found'});
  await replaceLinks(ctx,access,orgId,{table:'erp_accounting_dimension_type_module',idColumn:'accounting_dimension_type_id',idValue:r.rows[0].accounting_dimension_type_id,moduleIds:selectedModules});
